...
// Mimic a real SMTC snapshot: position 0:20 captured 10s ago, then FROZEN (WMC only
// re-reports on play/pause/seek). The widget must extrapolate the live position from
// the snapshot timestamp (playback_date/playback_time), not sit at 0:20.
const SNAP_EPOCH = Date.now() - 10000;
const SNAP_PROGRESS_MS = 20000;
...
createServer(async (req, res) => {
  ...
  if (path === '/' || path === '/status') {
    ...
    res.end(JSON.stringify({
      ...
      progress: SNAP_PROGRESS_MS, duration: t.duration, status: 'playing', source: t.app,
